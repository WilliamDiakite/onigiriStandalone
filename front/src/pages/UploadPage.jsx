...
export default class UploadPage extends Component {
  ...
  getFileList = () => {
    ...
    const rows = this.state.files.map((f, i) => {
      ...
      return (<tr className="dataset-row" key={i}>
        <td>{i + 1}</td>
        <td>{f.path}</td>
        <td>
          <Select name={`id-${i}`} options={labels} onChange={this.optionChange}/>
        </td>
        <td>
          <Select name={`match-${i}`} options={labels} onChange={this.optionChange}/>
        </td>
        <td>
          <Select name={`infoLabels-${i}`} options={labels} onChange={this.optionChange} isMulti="isMulti"/>
        </td>
        <td>{Math.round(f.size / 1000000, 3)}
          Mb</td>
      </tr>)
    })
    return rows
  ...
  uploadFiles = () => {
    var load = []
    for (var i = 0; i < 2; i++) {
      load.push({name: this.state.files[i].path, size: this.state.files[i].size, content: this.state.contents[i], options: this.state.options[i]})
    }
    ...
    return (<div className="page">
      ...
      <div style={{
        ...
        <div style={{
          ...
          <table className="pure-table" style={{
              marginTop: 40
            }}>
            <thead>
              <tr>
                <td style={{
                    width: 120
                  }}>Uploaded ({this.state.files.length}/2)</td>
                <td style={{
                    width: 150
                  }}>Filename</td>
                <td style={{
...

            {
              this.state.doUpload
                ? <Async promiseFn={this.uploadFiles}>
                    ...
                    <Async.Resolved>
                      {
                        data => (<div className="session-button">
                          <Link to={{
                              pathname: /harmony/ + this.state.session
                            }}>Start
                          </Link>
                        </div>)
                      }
